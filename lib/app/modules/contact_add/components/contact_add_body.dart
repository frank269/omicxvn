...
class ContactAddBody extends StatefulWidget {
  final widget = _ContactAddBodyState();
  getContact() => widget.getContact();

  @override
  State<ContactAddBody> createState() => widget;
}

class _ContactAddBodyState extends State<ContactAddBody> {
  getContact() => _contact;
  final _contact = ContactCreateParam();
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          buildCommonInfo(
              node, (value) => {_contact.fullName = value}, 'Họ và tên', true),
          buildCommonInfo(
              node,
          ...
          buildCommonInfo(
              node, (value) => {_contact.companyName = value}, 'Công ty'),
          buildCommonInfo(node, (value) => {_contact.occupationName = value},
              'Nghề nghiệp'),
          buildCommonInfo(node,
              (value) => {_contact.income = int.tryParse(value)}, 'Thu nhập'),
          buildCommonInfo(
              node, (value) => {_contact.address = value}, 'Địa chỉ'),
          buildCommonInfo(node, (value) => {}, 'Phương thức liên hệ mong muốn'),
          buildDropdownRow(
              node,
              ...
              dropDownMucdo),
          buildCommonInfo(
              node, (value) => {_contact.loan = value}, 'Nhu cầu vay'),
          buildCommonInfo(
              node, (value) => {_contact.assetType = value}, 'Tài sản vay'),
        ],
      ),
    );
  }

  buildCommonInfo(FocusScopeNode node, onEditing, String hint,
          [bool needValue = false]) =>
      Padding(
        padding: const EdgeInsets.only(bottom: 15.0),
        child: getTextField(node, onEditing, hint, needValue),
      );
      ...
      Padding(
        ...
        child: Row(
          ...
          children: [
            Expanded(
              flex: 1,
              child: getTextField(node, onEdited1, hint, needValue),
            ),
            SizedBox(
              width: 5,
            ),
  ...
  getTextField(FocusScopeNode node, onEditing, String hint,
          [bool needValue = false]) =>
      TextFormField(
